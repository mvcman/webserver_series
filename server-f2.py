    ...
    while True:
        ...
        if b'\r\n\r\n' in http_request:
            ...
            if re.search('Authorization:', my_req):
                ...
                fo = value[1].split()
                decode_data = decode_body(fo[1])
                user_data = decode_data.split(':')
                print(user_data, user_data[1])
                # print('decoded value ', decode_body(fo[1]))
                print('Decode value of user', create_hash(user_data[1]))
                print(create_hash('mandar'),create_hash('mandar1'), create_hash('mandar2'))
                user = user_data[0] + ':{0}'.format(create_hash(user_data[1]))
                print('My user is \n', user)
                if user in users:
                    print('found!')
                    isAuthenticated = 'yes'
